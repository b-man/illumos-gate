...
static void
dcache_clean(void *addr, size_t size)
{
	uintptr_t end = roundup((uintptr_t)addr + size, DCACHE_LINE);
	while ((uintptr_t)addr < end) {
		clean_data_cache_poc((uintptr_t)addr);
		addr = (void *)((uintptr_t)addr + DCACHE_LINE);
	}
	dsb();
}
...
static void
dcache_invalidate(void *addr, size_t size)
{
	uintptr_t end = roundup((uintptr_t)addr + size, DCACHE_LINE);
	while ((uintptr_t)addr < end) {
		invalidate_data_cache((uintptr_t)addr);
		addr = (void *)((uintptr_t)addr + DCACHE_LINE);
	}
	dsb();
}
...
emac_alloc_buffer(struct emac_sc *sc)
{
	...
	ddi_dma_attr_t tx_dma_attr = dma_attr;
	tx_dma_attr.dma_attr_align = DCACHE_LINE;
	tx_dma_attr.dma_attr_sgllen = 2;
...
emac_send(struct emac_sc *sc, mblk_t *mp)
{
	...
	dcache_clean(pkt->dma.addr, mblen);
	...
	for (;;) {
		...
		if (len > 0) {
			...
			if (pkt) {
				...
				*mblk_tail = mp;
				mblk_tail = &mp->b_next;
				dcache_invalidate(sc->rx_ring.pkt[index]->dma.addr, len);
				mp->b_wptr += len;
				sc->rx_ring.pkt[index] = pkt;
			}
		}

		{
			...
			dcache_invalidate(pkt->dma.addr, pkt->dma.size);
			desc_p->cntl = EMAC_DESC_CNTL_CHAIN | pkt->dma.size;
			desc_p->addr = pkt->dma.dmac_addr;
			dmb();
			desc_p->status = EMAC_DESC_STATUS_OWN;
