...
hati_mkpte(pfn_t pfn, uint_t attr, level_t level, uint_t flags)
{
	...
	if (!(attr & PROT_EXEC))
		PTE_SET(pte, (attr & PROT_USER)? PTE_UXN: PTE_PXN);
...
#define	PTE_REMAP_BITS							\
	(PTE_PFN_MASK | PTE_UXN | PTE_PXN | PTE_AF | PTE_AP_RO | PTE_ATTR_MASK)
...
try_again:
		...
		if (what == HAT_SET_ATTR) {
			...
			if (PTE_GET(oldpte, PTE_AP_USER)) {
				if ((attr & PROT_EXEC) && PTE_GET(oldpte, PTE_UXN))
					newpte &= ~PTE_UXN;
			} else {
				if ((attr & PROT_EXEC) && PTE_GET(oldpte, PTE_PXN))
					newpte &= ~PTE_PXN;
			}
		}
