...
static void
meson_set_mcr(struct asycom *asy, uint8_t mcr)
{
	union uart_cr reg;
	reg.dw = REG_READ(asy, UARTCR);
	reg.dtr = ((mcr & DTR)? 1: 0);
	reg.rts = ((mcr & RTS)? 1: 0);
	reg.out1 = ((mcr & OUT1)? 1: 0);
	reg.out2 = ((mcr & OUT2)? 1: 0);
	reg.lbe = ((mcr & ASY_LOOP)? 1: 0);
	REG_WRITE(asy, UARTCR, reg.dw);
}

static uint8_t
meson_get_mcr(struct asycom *asy)
{
	union uart_cr reg;
	reg.dw = REG_READ(asy, UARTCR);
	uint8_t ret = 0;
...
meson_set_icr(struct asycom *asy, uint8_t icr, uint8_t mask)
{
	...
	REG_WRITE(asy, UARTIMSC, reg.dw);
}

static uint8_t
meson_get_icr(struct asycom *asy)
{
	union uart_intr reg;
	reg.dw = REG_READ(asy, UARTIMSC);
	uint8_t ret = 0;
...
static uint8_t
meson_get_lsr(struct asycom *asy)
{
	union uart_rsr rsr;
	rsr.dw = REG_READ(asy, UARTRSR);
	uint8_t ret = 0;
	if (rsr.fe)
		ret |= FRMERR;
	if (rsr.pe)
	...
	if (rsr.oe)
		ret |= OVRRUN;
	REG_WRITE(asy, UARTECR, 0);

	union uart_fr fr;
	fr.dw = REG_READ(asy, UARTFR);
	if (fr.txfe) {
		ret |= XHRE;
		if (!fr.busy)
			ret |= XSRE;
...
{
	union uart_dr reg;
	reg.dw = REG_READ(asy, UARTDR);
	return reg.data;
}

static void
meson_set_lcr(struct asycom *asy, uint8_t lcr)
{
	union uart_lcr_h reg;
	reg.dw = REG_READ(asy, UARTLCR_H);
	...
	REG_WRITE(asy, UARTLCR_H, reg.dw);
}

static void
meson_set_break(struct asycom *asy, boolean_t on)
{
	union uart_lcr_h reg;
	reg.dw = REG_READ(asy, UARTLCR_H);
	reg.brk = (on? 1: 0);
...
static uint8_t
meson_get_lcr(struct asycom *asy)
{
	union uart_lcr_h reg;
	reg.dw = REG_READ(asy, UARTLCR_H);
...
static uint8_t
meson_get_isr(struct asycom *asy)
{
	union uart_intr reg;
	reg.dw = REG_READ(asy, UARTRIS);
...
{
	REG_WRITE(asy, UARTCR, 0);
	REG_WRITE(asy, UARTECR, 0);
	REG_WRITE(asy, UARTIMSC, 0);
	REG_WRITE(asy, UARTICR, 0xffff);

	union uart_ifls ifls = {0};
	ifls.txiflsel = 2;	// 1/2
	ifls.rxiflsel = 0;	// 1/8
	REG_WRITE(asy, UARTIFLS, ifls.dw);

	union uart_cr cr = {0};
	cr.uarten = 1;
	cr.txe = 1;
	cr.rxe = 1;
	REG_WRITE(asy, UARTCR, cr.dw);

	union uart_lcr_h lcr_h = {0};
	lcr_h.fen = 1;
	lcr_h.wlen = 3;
	REG_WRITE(asy, UARTLCR_H, lcr_h.dw);
}
...
meson_set_baud(struct asycom *asy, uint8_t bidx)
{
	...
	uint32_t bauddiv = (asy->asy_clock * 4 + baudrate / 2) / baudrate;

	uint32_t cr = REG_READ(asy, UARTCR);
	REG_WRITE(asy, UARTCR, 0);
	...
	REG_WRITE(asy, UARTIBRD, ibrd.dw);
	REG_WRITE(asy, UARTFBRD, fbrd.dw);
	REG_WRITE(asy, UARTCR, cr);
}
...
asyattach(dev_info_t *devi, ddi_attach_cmd_t cmd)
{
	...
	ddi_prop_update_string(dev, devi, "uart", asy_hw_name(asy));

	meson_reset(asy);

	/* Set the baud rate to 9600 */
	meson_set_baud(asy, asy->asy_bidx);
	meson_set_lcr(asy, asy->asy_lcr);
	meson_set_mcr(asy, mcr);
...
nodrain:
	...
	if ((async->async_ttycommon.t_cflag & HUPCL) ||
		...
		if (asy->asy_flags & (ASY_IGNORE_CD|ASY_RTS_DTR_OFF)) {
			...
			meson_set_mcr(asy, asy->asy_mcr|OUT2);
		} else {
			DEBUGCONT1(ASY_DEBUG_MODEM,
...
asy_program(struct asycom *asy, int mode)
{
	...
	if ((CRTSCTS & async->async_ttycommon.t_cflag) && !(asy->asy_msr & CTS)) {
...
asyintr(caddr_t argasy)
{
	...
	interrupt_id = meson_get_isr(asy);
	async = asy->asy_priv;
	...
	if ((async == NULL) ||
		...
		if (interrupt_id & NOINTERRUPT)
			 ...
			 */
			(void) meson_get_lsr(asy);
			(void) meson_get_char(asy);
			asy->asy_msr = meson_get_msr(asy);
	...
	if (async->async_ext) {
		...
		if (asy->asy_msr & DCD) {
			...
			if ((async->async_flags & ASYNC_CARR_ON) &&
				 ...
				 */
				val = meson_get_mcr(asy);
				meson_set_mcr(asy, (val & ~DTR));
...
asymctl(struct asycom *asy, int bits, int how)
{
	...
	case TIOCMGET:
		 ...
		 */
		if (meson_get_icr(asy) & MIEN) {
			msr_r = asy->asy_msr;
			DEBUGCONT2(ASY_DEBUG_MODEM,
			    "asy%dmctl: TIOCMGET, read msr_r = %x\n",
			    instance, msr_r);
		} else {
			msr_r = meson_get_msr(asy);
			DEBUGCONT2(ASY_DEBUG_MODEM,
