    ...
    uint_t max_segs)
{
	...
	virtio_put32(vio, VIRTIO_MMIO_QUEUE_ALIGN, VirtIoQueueAlign);
	qsz = virtio_get32(vio, VIRTIO_MMIO_QUEUE_NUM_MAX);
	qsz = MIN(qsz, VirtIoQueueNumMax);
	if (qsz == 0) {
		/*
	...
	ASSERT(((uintptr_t)virtio_dma_va(&viq->viq_dma, 0) & (VirtIoQueueAlign - 1)) == 0);
