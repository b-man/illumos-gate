...
int plat_hwreset_assert(struct prom_hwreset *rst)
{
	...
	if (get_reg_addr(rst->node, 0, &base) != 0)
		return -1;
	*(volatile uint32_t *)(base + ((0x50 + rst->id / 32) << 2)) &= ~(1u << (rst->id % 32));
	return 0;
}
...
int plat_hwreset_is_asserted(struct prom_hwreset *rst, bool *asserted)
{
	uint64_t base;
	if (get_reg_addr(rst->node, 0, &base) == 0)
		return -1;
	*asserted = ((*(volatile uint32_t *)(base + ((0x50 + rst->id / 32) << 2)) & (1u << (rst->id % 32))) == 0);
	return 0;
}
